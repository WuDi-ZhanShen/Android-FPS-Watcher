...
public class MainActivity extends Activity {
    ...
    private final BroadcastReceiver mContentProviderBroadcastReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {

            Toast.makeText(context, R.string.activate_success, Toast.LENGTH_SHORT).show();
            Handler handler = new Handler(Looper.getMainLooper());
            // 主线程更新 UI
            handler.post(MainActivity.this::setButtonEnabled);
        }
    };
    ...
    private void check() {
        ...
        if (b && c) {
            try {
                ...
                String cmd = getAppopsCmd() +
                        getPmCmd() +
                        "export CLASSPATH=" + getExternalFilesDir(null).getAbsolutePath() + "/classes.dex;" +
                        "nohup app_process / " + Watch.class.getName() + " > /dev/null 2>&1 &";
...
//        }
        ...
        registerReceiver(mContentProviderBroadcastReceiver, new IntentFilter(MyProvider.ACTION_BINDER_RECEIVED), Context.RECEIVER_EXPORTED);
    ...
    private void setButtonsOnclick(boolean isNight, SharedPreferences sp) {
        ...
        switch3.setChecked(sp.getBoolean("enable_notification", true));
        switch3.setOnCheckedChangeListener((buttonView, isChecked) -> sp.edit().putBoolean("enable_notification", isChecked).apply());
        TextView textView3 = findViewById(R.id.text3);
        textView3.setOnClickListener(v -> showNotificationHelp());

        Switch switch4 = findViewById(R.id.switch4);
        switch4.setChecked(sp.getBoolean("enable_float_window", true));
        switch4.setOnCheckedChangeListener((buttonView, isChecked) -> sp.edit().putBoolean("enable_float_window", isChecked).apply());
    ...
    private void showNotifyForeChangeHelp() {
        new AlertDialog.Builder(this)
                .setTitle(R.string.notify_fore_change)
                .setMessage(R.string.notify_fore_change_content)
    ...
    private void showNotificationHelp() {
        new AlertDialog.Builder(this)
                .setTitle(R.string.enable_notification)
                .setMessage(R.string.enable_notification_content)
                .setNegativeButton(R.string.understand, null)
                .show();
    }

    private void showFloatWindowHelp() {
        new AlertDialog.Builder(this)
                .setTitle(R.string.enable_float_window)
                .setMessage(R.string.enable_float_window_content)
                .setNegativeButton(R.string.understand, null)
                .show();
    }
...
//        });
        button.setOnClickListener(view -> {
            SharedPreferences sp = getSharedPreferences("s", 0);
//            if (!sp.getBoolean("aware_tile", false)) {
//                new AlertDialog.Builder(this)
//                        .setTitle(R.string.aware_title_launch)
//                        .setMessage(R.string.aware_title_launch_content)
//                        .setNegativeButton(R.string.never_show_again, (dialog, which) -> sp.edit().putBoolean("aware_tile", true).apply())
...
//            }
    ...
    protected void onDestroy() {
        ...
        unregisterReceiver(mContentProviderBroadcastReceiver);
        super.onDestroy();
    }
    ...
    private String getAppopsCmd() {

        String[] permissionsToAllow = {
                "RUN_IN_BACKGROUND",
        ...
        StringBuilder appopsCmdBuilder = new StringBuilder();
        for (String permission : permissionsToAllow) {
            appopsCmdBuilder.append("cmd appops set ")
                    .append(packageName)
                    .append(" ")
                    .append(permission)
                    .append(" allow;");
        }
        return appopsCmdBuilder.toString();
    }

    private String getPmCmd() {
        ...
        StringBuilder appopsCmdBuilder = new StringBuilder();
        for (String permission : permissionsToAllow) {
            appopsCmdBuilder.append("pm grant ")
                    .append(packageName)
                    .append(" ")
                    .append(permission)
                    .append(";");
