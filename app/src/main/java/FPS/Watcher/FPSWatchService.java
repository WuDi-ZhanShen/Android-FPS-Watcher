    ...
    private final IFpsCallback iFpsCallback = new IFpsCallback.Stub() {
        ...
        public void onTargetTaskChanged(String packageName) {
            ...
            try {
                ...
                if (notification != null) {
                    notification.setContentTitle(getString(R.string.watch_target) + label);
                    notificationManager.notify(1, notification.build());
                }
        ...
        public void onReceive(Context context, Intent intent) {
            switch (intent.getAction()) {
                case Watch.SEND_BINDER_ACTION:
                    ...
                    } catch (RemoteException | PackageManager.NameNotFoundException ignored) {
...
//                        sharedPreferences.edit().putInt("menuX", params.x).putInt("menuY", params.y).apply();
    ...
    private void saveFpsDataToFile() {
        ...
        try {
            ...
            ApplicationInfo appInfo = pm.getApplicationInfo(mWatchingPackageName, 0);
            String label = pm.getApplicationLabel(appInfo).toString();
            String filename = sdf2.format(System.currentTimeMillis()) + "_" + label + ".csv";
            File recordFolder = new File(getExternalFilesDir(null), "FPSRecord");
